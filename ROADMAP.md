...
### NC7 — Planner Integration ✅(2026-09-12/13 夜功能面收官)

- **Status**:✅ Done(首片 2026-09-11/12:recommendation 可绑定 NC3 SkillDefinition,create-task 产出 executor=auto 任务——native skill 可用即走 native,否则回退绑定路线的外部命令;`POST /attach-skill` + ctl + 看板「绑定 Skill」全通。二片 2026-09-12/13:①执行反馈统计——`GET /api/planner/recommendations/{id}/feedback` + ctl `planner feedback` + 看板「反馈」弹窗,对推荐关联任务做只读执行 rollup[总数/成功/失败/取消/进行中/最近执行 vs `estimated_runs`],**语义有意保守**:不自动改 `owned_count`、不动推荐生命周期;②无路线纯 skill 推荐——create-task 对「有 skill 无路线」产出**纯 native 任务**[`type=native`/`executor=native`,无外部回退分支]。验收映射:推荐一键生成 native 任务 ✅(api 测试+实机 smoke),结果 dashboard 可见 ✅(反馈弹窗+执行视图),执行链路 ✅(windows_smoke [12]-[17] 真实 controller 会话/取消/auto 双分支)。)
- **Objective**:打通 `Planner → Recommendation → Skill/Route → Native Controller`,执行结果回流 Execution。
- **Scope**:recommendation 增加可选 skill 绑定 ✅;执行结果(feedback)反哺 planner 统计 ✅(只读 rollup,材料入账语义明确排除);无路线纯 skill 推荐 ✅(native-only 形态,有路线仍走 auto 双分支)。
...
- **Acceptance Criteria**:
  ...
  - 该草案在 dry-run 回放中完整走查并输出计划动作轨迹(与 NC3 轨迹日志/TSV 联动)——✅ 2026-09-13/14 夜真值闭环:e2e 此前只断言 outcome=done(duration 到期即 done,卡死的 walk 被掩盖),soak 抓出 step_01 卡死后修复学习端缺陷[锚点排除消失侧+主色改众数],e2e 现断言轨迹 step_01→…→done;
