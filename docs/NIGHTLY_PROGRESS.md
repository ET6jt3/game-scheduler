...
| M8 | **双证据轮**:①NC9 链式夹具 e2e 对今晚 controller 变更全绿(--keep 留档);②30s ONNX cache soak PASS(447 周期,59 命中,done);③**30min 协议 soak**(NC9 夹具 replay+skill,3fps):5362 周期,D1 EVENT 恰 4 条(done 恰 1),零解析失败;**M5 判别:stderr `skill: DONE` 0 行**(修复前~5300);M7 后命中 5190/5361=96.8% 与历史持平,inference_count=171≈5362/32 强制刷新吻合;**额外活体验证:duration(1800s)与 governor max_session(30min)同刻到期→outcome=stopped 优先于 done——安全停止优先契约(e1ae286)在大规模下正确裁决** | PASS | (证据轮,M5/M7 的 commit) | .nightly/soak-proto-m5m7.{out,err}.log+tsv |
...
| M5 | **语义修订**:skill 终态失败必须传导到 session outcome——原实现只跟踪会话级结果,skill 走到 Failed 终态后 RESULT 仍 outcome=done(有测试**显式钉死**该行为),Go 侧 done→success 使失败重试/失败通知/失败截图/NC7 反馈统计全部失效。现 RESULT outcome=skill-failed(wire Failed)+error 注明失败状态;governor/input-blocked 停止保持优先;成功路径不变(链式 e2e 复验轨迹到 done);协议草案+双语 README 同步 | PASS | 5e5a02e | 修订后 failed_skill_event_emits_once 断言 FAILED+error 文本(原测试断言 done 已改);133 lib+协议集成 3/3;clippy 0;LOCAL CI PASS |
...
| M10 | 过夜 Go 侧调度链 soak(385min,cron 每分钟):**385/385 fires 精确命中,failed_24h=0,running=0 全程,峰值 WS 22.1MB**(16.5→22.1 缓增 ~1MB/h,与保留行数增长相称);dashboard totals 口径逐分钟 386 行采样留档;挂具自带终验断言 exit 0 | PASS | (证据轮) | samples.csv + harness 终验 |
...
| M6 | **Go 侧调度链 2h 回归 soak**(fe86826 重建 server/ctl):raw 任务 @every 10s 点火 7200s。**质量信号全绿**:全程采样 id 单调推进(10→46→258→326→369→614,节奏 ~10s/次)、最新 500 条收尾时 100% success、零 failed、零 stuck、资源平坦(WS 20.9→22.6MB,CPU 累计 1.8s)。挂具自判 FAIL 系**测量口径缺陷非调度缺陷**:①`execs list` 被 API 500 行上限钳制,720 次点火只能列出最新 500;②scratch 挂具的期望常量沿用 12600s 口径(~1260)未随 7200s 换算。真实总火数因挂具清理了临时库不可复原,按 id 外推 ~730 与期望吻合 | PASS(质量)/挂具口径缺陷如实记录 | (无代码变更) | 采样见 M8;临时库已被挂具清理,证据为本行记录 |
