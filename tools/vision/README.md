...
- 指标敏感性：全局均值抓整屏切换，峰值块阈值抓局部 UI 变化（弹窗/
  ...
  的事件）。两阈值均可 CLI 调。
- 锚点与主色质量（2026-09-13/14 夜 soak 抓出的缺陷修复）：锚点只保留
  **有新内容**的块（进入段时「旧内容消失」也是强差异——不排除会把上
  一场景的残影并进锚点，产生 0.75×0.75 的退化大锚点）；主色取区域
  **众数**而非均值（均值会把「67% 红块+33% 背景」混成 (146,40,40) 这
  种任何实际像素都不匹配的颜色，探针永不触发）。selftest 现校验每个
  探针在自己的段帧上**能触发且对上一段帧不触发**。
