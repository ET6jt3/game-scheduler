...
- **角色培养计划器**:手动维护角色/目标/材料缺口,按路线资产生成刷取推荐,并一键创建任务/计划;推荐未匹配到路线时可**手动绑定**,支持整体**导出/导入**(单事务原子导入)。
- **执行日志保留**:已结束的执行按 `execution_retention_days`(默认 30)自动清理,数据库不无限膨胀。
- **一键打包发布**:推 `v*` 标签自动构建 Windows/Linux 压缩包并发布 GitHub Release;`scriptsuild.ps1` 本地同款打包。
- **截图辅助接口预留**(internal/vision):Detector / Matcher / OCR / FrameSource 纯接口,供未来扩展,当前不含任何模型或推理。
...
> 🛡️ **必须以管理员身份运行(否则 BetterGI 等会报退出码 553)。** 这些工具靠模拟鼠标键盘操作游戏,而游戏本身以管理员权限运行,所以工具(以及启动它们的本调度器)也必须是管理员。子进程只能继承父进程的权限,**因此 `server.exe` 本身要用管理员启动**。便捷脚本:`powershell -ExecutionPolicy Bypass -File examples\run-admin.ps1`(会弹 UAC,点「是」)。非管理员启动时,server 日志会打印一条告警提示。

> 📁 **工作目录自动默认**:若游戏未填 `working_dir` 且 `tool_path` 是绝对路径,调度器会自动把工作目录设为**该 exe 所在目录**——BetterGI / ok-ww 这类 GUI 工具要在自己安装目录下才能找到资源(否则也会 553)。需要时仍可用游戏的 `working_dir` 或任务 params 的 `working_dir` 显式覆盖。
...
cargo run -- --dry-run --backend synthetic --model-path tests\fixtures\constant_yolo.manifest.json  # 走真实 ONNX(有权重时)/manifest 感知
# 注意:实际吞吐受每周期捕获成本限制(PrintWindow 强制渲染 ~280ms/640x480),
# --fps 只是节流上限;高吞吐捕获走 WGC(性能路径)。
...
#            --model-path <manifest.json>(manifest 的 imgsz/置信度生效,CLI 显式旗标优先)
```

一键冒烟(窗口查找 → GDI 捕获+检测 → manifest 门禁 → ONNX dry-run → plain dry-run,逐项校验退出码):
...
> 💡 **绑定 Skill(NC7)**:推荐还可以绑定一个 NC3 SkillDefinition 文件:`POST /api/planner/recommendations/{id}/attach-skill` + `{"skill":"skills/daily.json"}`(文件必须已存在;ctl 用 `planner attach-skill <推荐id> -skill <路径>`;看板推荐表点「绑定 Skill」)。绑定后创建的任务分两种:**有绑定路线 → `executor=auto`**,native controller 可用就走 skill,不可用自动回退到绑定的路线命令——只降级不升级,真实输入双闸照旧;**无路线的纯 skill 推荐 → 纯 native 任务**(`type=native`/`executor=native`),没有外部回退分支,调度侧按 native 契约(preflight + 双闸)执行。**先建任务后绑 skill 也生效**:绑定会回灌进推荐已关联的任务(显式 executor 优先,无选择器则补 `auto`,原外部命令保持为回退分支)。看板推荐表会显示 `skill` 徽标与文件路径。

> 💡 **执行结果反馈(NC7)**:`GET /api/planner/recommendations/{id}/feedback`(ctl:`planner feedback <推荐id>`)返回推荐关联任务的执行统计 rollup:总执行数、成功/失败/取消、进行中、最近一次执行(状态与时间)、以及推荐的预估次数(`estimated_runs`)。**语义是有意保守的**:这是观察事实的汇总,不是材料入账——一次成功 run 不等于材料已到手,所以该端点**不会**自动修改需求的 `owned_count`、也不会改动推荐的生命周期状态;是否标记完成、是否更新库存始终是人工决定。统计覆盖保留的执行历史(执行记录按保留策略清理,数字描述的是仍可观测的部分);推荐关联的任务被删除时链接会自动置空,返回零统计。看板「培养计划 → 推荐」每行有「反馈」按钮,弹窗直接展示这份统计。
...
ctl -server $S planner feedback <推荐id>   # 执行结果统计(NC7,只读不改库存)
```
...
planner  recommend | recommendations [-goal id] [-game id] [-status s] [-limit n]
         | create-task <id> | create-plan <id> | attach-route <id> -route <路线id>
