...
6. Choose stop on failure (default) or continue to the next game. Retries and retry delays come from each task's existing settings. Ordinary chain steps wait for natural completion unless explicitly configured with a hard timeout. Lifecycle-sensitive helper definitions may retain a bounded timeout so a broken helper cannot stall the entire chain indefinitely.
...
For the built-in ok-nte integration, normal task runs do not use the PyAppify ok-nte.exe window as the lifecycle owner. Game Scheduler launches the packaged Python interpreter with a scheduler-owned in-memory compatibility bootstrap. The bootstrap constructs ok.OK(config), emits the runtime-ready start_success event that the packaged headless task path otherwise skips, waits for ok-nte's OpenVINO RuntimeServices detector to initialize, and then runs one-time task 2 (DailyRoutineTask) with exit-after semantics. ok-script still enables NTE's hidden LauncherTask before DailyRoutineTask; that launcher task finds or starts the native NTE launcher, handles its Update/Start Game flow, waits for HTGame.exe and capture readiness, and then daily work proceeds in the same Python process. The bootstrap forces UTF-8 stdout/stderr and converts DailyRoutineTask failed or pending sub-items into process exit code 20, so Game Scheduler no longer records an internally failed daily routine as success. main.py remains a required preflight file but is not executed directly. A six-hour fallback timeout is retained inside chains so a broken worker cannot stall forever. Preserved older Config/helpers/ok-nte.json files are ignored for this reserved built-in ID so portable upgrades cannot silently revert to the obsolete launcher-only contract. Real game behavior still needs testing on your desktop.

## Update an existing portable installation without losing configuration

Stop the old package with its `Stop.cmd` and back up that package first. Build the updated branch with `Build.cmd`, or download the Windows acceptance ZIP. To retain the active package's settings and history, copy the new package's **App** files plus **Start.cmd**, **Stop.cmd**, **Setup-Startup.cmd**, and **Remove-Startup.cmd** into the old package. Keep the old **Config**, **Data**, **Helpers**, and **Runtime** directories. Do not copy the new package's empty Data or example Config over your active installation. The reserved built-in ok-nte manifest remains authoritative even when an older copied Config/helpers/ok-nte.json remains on disk.
