...
For the built-in ok-nte integration, normal task runs do not use the PyAppify ok-nte.exe window as the lifecycle owner. They run the packaged Python worker headlessly as main.py -t DailyRoutineTask -e -h. ok-script enables NTE's hidden LauncherTask before the requested DailyRoutineTask; that launcher task finds or starts the native NTE launcher, handles its Update/Start Game flow, waits for HTGame.exe and capture readiness, and then the daily task runs in the same worker process. Completion is the Python worker's own process exit after ok-script's -e success path, not either launcher window. A six-hour fallback timeout is retained inside chains so a broken worker cannot stall forever. Preserved older Config/helpers/ok-nte.json files are ignored for this reserved built-in ID so portable upgrades cannot silently revert to the obsolete launcher-only contract. Real game behavior still needs testing on your desktop.
